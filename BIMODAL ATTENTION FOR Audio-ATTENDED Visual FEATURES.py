...
# ========== CONFIGURATION ==========
visual_features_root = "/home/dgx1user3/Localization/visual_features_resnet"
...
video_lists = {}
for split in splits:
    split_dir = os.path.join(visual_features_root, split)
...
# ========== STEP 5: SIMPLE VISUAL PROCESSOR ==========
...
#     def __init__(self, visual_dim=2048, audio_dim=128, output_dim=512):
#         super(BimodalAttention, self).__init__()
#         self.audio_proj = nn.Linear(audio_dim, visual_dim)
#         self.cross_attention = nn.MultiheadAttention(embed_dim=visual_dim, num_heads=4, batch_first=True)
#         self.visual_norm = nn.LayerNorm(visual_dim)
#         self.audio_norm = nn.LayerNorm(visual_dim)
#         self.fc = nn.Linear(visual_dim, output_dim)
#     def forward(self, visual_feat, audio_feat):
#         visual_feat = self.visual_norm(visual_feat).unsqueeze(0)
#         audio_feat = self.audio_proj(audio_feat)
#         audio_feat = self.audio_norm(audio_feat).unsqueeze(0)
#         attn_output, _ = self.cross_attention(visual_feat, audio_feat, audio_feat)
#         attn_output = self.visual_norm(attn_output + visual_feat).squeeze(0)
#         output = attn_output.mean(dim=0)
#         output = self.fc(output)
#         return output

# ========== STEP 8: PERFORM BIMODAL ATTENTION ==========
# (Commented out as per user request)
# def perform_bimodal_attention():
...
#         video_ids = sorted([f.replace(".npy", "") for f in os.listdir(v_dir) if f.endswith('.npy')])
...
#                 continue
#             visual_feat = torch.tensor(np.load(v_path), dtype=torch.float32).to(device)
#             audio_feat = torch.tensor(np.load(a_path), dtype=torch.float32).to(device)
#             with torch.no_grad():
#                 output_vec = model(visual_feat, audio_feat)
#             np.save(out_path, output_vec.cpu().numpy())
